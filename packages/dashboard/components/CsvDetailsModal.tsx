...
const CsvDetailsModal: React.FC<WebsiteDetailsModalProps> = ({
  open,
  onClose,
  data
}) => {
  ...
  const formatAddress = (address: string) => {
    try {
      ...
      return {
        formatted: `${street || ''}, ${city || ''}, ${state || ''}, ${country || ''}`.replace(/^,\s*|,\s*$/g, '').replace(/,\s*,/g, ','),
        street: street || '',
        city: city || '',
        state: state || '',
        country: country || ''
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
              ...
              {data.service_name && (
                <Box sx={{ flex: 1 }}>
                  ...
                  <Chip label={data.service_name} size="small" color="secondary" />
                </Box>
              )}
            </Box>
          </Box>
